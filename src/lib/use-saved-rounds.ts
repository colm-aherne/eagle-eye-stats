...
export function useSavedRounds() {
      ...
      if (session?.user) {
        ...
        if (local.length > 0) {
          const { error: migrateError } = await supabase
            .from("rounds")
            .upsert(local.map((r) => toRow(r, session.user.id)));
          if (migrateError) {
            console.error("Failed to migrate local rounds to cloud", migrateError);
            toast.error("Couldn't sync local rounds to the cloud — try again");
          } else {
  ...
  const upsert = useCallback(
    async (r: Round) => {
      ...
      if (session?.user) {
        const { error } = await supabase.from("rounds").upsert(toRow(r, session.user.id));
        if (error) {
          console.error("Failed to save round to cloud", error);
          toast.error("Couldn't save to the cloud — try again");
          throw error;
        }
      }
    },
